...
function createForm() {
  ...
  if (!formActive) {
    while (count > 0) {
      ...
      errorText.className = 'error-field'
      newEntry.appendChild(errorText)
      if (count == 1) {
        ...
        readRadio.setAttribute("onclick", 'setRadio("read")');
      ...
      if (count == 1) {
        ...
        unreadRadio.setAttribute("onclick", 'setRadio("unread")');
...
function submitBook() {
  let myErrors = document.querySelectorAll('.error-field')
  let myVals = document.querySelectorAll(".new-input");
  let myForm = document.querySelector('.new-entry-forms')
    for (i = 0; i < 2; i++) {
